...
`ConsoleBridgeServer` 不捕获 I/O 异常做静默降级。`Dispose`、`AcceptLoop`、`HandleConnection` 和 `FlushPendingOutput` 中的 I/O 操作若失败，异常直接传播。客户端断连产生的异常通过 `when (_cts.IsCancellationRequested)` 精确捕获，系统级 socket 错误和写入失败不再被静默吞掉。
