...
| 能力 | 说明 | 文档入口 |
|------|------|----------|
| DataSource 抽象层 | 统一树数据模型 DataSourceNode（Object/Array/String/Number/Boolean/Null + Lazy），通过 IDataSourceIoGateway 作为 Core 层唯一文件入口 | [↔ DataSource](../Origo.Core/DataSource/README.md) |
...
| 2D 噪声图生成 | OpenSimplex2 (70%) + Worley Cellular (30%) 混合噪声，可配置种子与频率 | [↔ Random](../Origo.Core/Random/README.md) |
| 内存黑板 | IBlackboard 默认实现，Set/TryGet/SerializeAll/DeserializeAll，key 大小写敏感 | [↔ Blackboard](../Origo.Core/Blackboard/README.md) |
...
| 接口隔离（ISP） | ISndContext 拆分为 10 个窄角色接口，ISessionRun 返回抽象 IStateMachineContainer | [架构概览](architecture-overview.md) |
