...
| **冲刷 Core 延迟管线** | 帧末操作（业务延迟队列、系统延迟队列、KillPendingEntities）的时机和顺序由 Core 控制 | 适配层不得直接调用 `FlushDeferredActionsForCurrentFrame()` |
| **实现 Core 内部接口** | `IEntityLifecycle` 等 `internal` 接口为框架内部使用，适配层不应直接实现 | `GodotSndEntity` 实现 `IEntityLifecycle` 仅为桥接委托（透传至内部 `SndEntity`） |
...
| **延迟动作管线** | `ActionScheduler` 业务队列 + 系统队列，`OrigoAutoHost._Process` 统一冲刷 |
| **持久化与序列化** | 存档读写、场景序列化、黑板持久化全部在 Core 完成 |
...
Godot._Process
  └── OrigoAutoHost._Process          ← 适配层（唯一帧入口）
        ├── SndRuntime.ProcessAll()   ← Core: 实体 Process 钩子
        ├── Runtime.FlushEndOfFrameDeferred() ← Core: 延迟动作 + KillPendingEntities
        └── Console.ProcessPending()  ← Core: 控制台
```

帧循环的入口在适配层（Godot 的 `_Process` 回调），但进入后立即将控制权交给 Core 层。适配层不参与帧内的任何逻辑决策。
