...
| `Dead()` | 死亡销毁（帧末统一执行） | BeforeDead → Teardown |
| `Process(delta)` | 每帧更新 | Process（按优先级 + 快照迭代）|

`IsPendingKill` 标记由 `RequestKillEntity()` 立即设置，`Dead()` 仅由框架统一 Kill 步骤调用。业务代码通过 `ISndContext.RequestKillEntity()` / `RequestKillAll()` 标记实体，由帧末 `KillPendingEntities()` 统一执行 `Dead()`。
