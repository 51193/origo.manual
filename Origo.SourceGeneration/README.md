...
| `AnalyzerReleases.Unshipped.md` | 分析器发布跟踪（未发布规则：`ORIGOSG001`、`ORIGOSG002`、`ORIGOSG003`） |
...
| `ORIGOSG003` | Error | 注册类型的 `startKind + count` 溢出 `byte` 范围（超过 255），导致 Kind 值被截断为 0 从而与 Null 哨兵冲突。 |
